...
export const useBusinessPlan = (userId: string | undefined, projectId: string | undefined, stepProgress: Record<string, StepProgress>) => {
  ...
  // Auto-generate content from all project data
  const generateContent = useCallback(async () => {
    if (!projectId || !stepProgress) return null;

    try {
      // Fetch project, simulation, channels, costs in parallel
  ...
  // Auto-generate on first load if no saved content exists
  useEffect(() => {
    if (!loading && projectId && stepProgress) {
      const hasContent = businessPlan.executive_summary || businessPlan.company_description || 
                         businessPlan.market_analysis || businessPlan.organization ||
                         businessPlan.products_services || businessPlan.marketing_strategy ||
                         businessPlan.financial_plan;
      
      if (!hasContent) {
        generateContent();
      } else {
        // Still generate for reference, but don't overwrite saved content
        generateContent();
      }
    }
